...
class api_endpoints(db.Model):
    ...
    api_provider = db.relationship(
        "api_providers", backref=db.backref('api_endpoints', order_by=id))
...
class api_authent_info(db.Model):
    ...
    api_endpoint = db.relationship(
        "api_endpoints", backref=db.backref('api_authent_info', order_by=id))
...
class api_authent_terms(db.Model):
    ...
    api_endpoint_id = db.Column(db.Integer, db.ForeignKey('api_endpoints.id'))
    api_endpoint = db.relationship(
        "api_endpoints", backref=db.backref('api_authent_terms', order_by=id))
