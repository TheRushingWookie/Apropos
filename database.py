...
def verify_owner(api_provider_name,input_json):
    ...
    owner_key = api_providers.query.filter_by(api_provider_name=api_provider_name).one().owner_key
    ...
    if owner_verified:
        return True
    logger.debug('''owner_key: %s . provided_hmac %s .wanted hmac %s . api_provider_name %s . owner_verified %s .''', owner_key, hmac, dbhmac, api_provider_name, owner_verified )
    return False
...
def update_tags(api_provider_name,api_endpoint_name,new_tags,json_input):
    ...
    if owner_verified:
        ...
        logger.warning('Unverified update_tags %s %s.', api_provider_name,api_endpoint_name)
